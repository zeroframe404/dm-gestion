...
import type { ImagenDeReporte, ReporteCreado } from '../../shared/tipos'
...
export function DialogoReportarError({ abierto, alCerrar }: { abierto: boolean; alCerrar: () => void }) {
  ...
  return (
    <Dialogo
      abierto={abierto}
      titulo={creado ? 'El reporte quedó anotado' : 'Reportar un error'}
      descripcion={
        creado
          ? 'Ya está del otro lado. No hace falta que lo mandes por ningún otro lado.'
          : 'Contá qué pasó y, si podés, adjuntá una captura. El programa agrega solo tu nombre, tu sucursal y la versión.'
      }
      alCerrar={alCerrar}
      ancho="lg"
      pie={
    ...
    >
      {creado ? (
        <div className="flex flex-col gap-3">
          <Alerta tono="exito">
            Quedó anotado como el reporte <strong className="font-semibold">#{creado.numero}</strong>
            {creado.imagenesSubidas > 0 && <>, con {creado.imagenesSubidas === 1 ? 'una captura' : `${creado.imagenesSubidas} capturas`}</>}.
          </Alerta>
          <p className="text-sm leading-relaxed text-slate-600">
            Si el problema te deja seguir trabajando, seguí: esto ya llegó. Si te frena, avisá por teléfono además, porque nadie
            está mirando la lista de reportes todo el día.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          <Campo
            ...
            maxLength={160}
            autoFocus
            ayuda="Una línea. Con la pantalla y qué estabas haciendo alcanza: «Cobranzas: el ticket sale sin la dirección»."
          />
          <AreaTexto
            etiqueta="Qué pasó"
            value={cuerpo}
            ...
            rows={6}
            ayuda="Lo que hiciste, lo que esperabas que pasara y lo que pasó. Si aparece un cartel de error, copiá el texto tal cual."
          />
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-slate-500">
                {imagenes.length}/{MAXIMO_IMAGENES} · opcionales, pero son lo que más ayuda
              </span>
