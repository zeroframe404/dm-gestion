...
export function Galeno() {
  ...
  const [estado, setEstado] = useState<EstadoDeGalenoNovedades | null>(null)
  ...
  const cargar = useCallback(async () => {
    ...
    const [respuestaEstado, respuestaBandeja] = await Promise.all([
      window.dm.galenoNovedades.estado(),
      window.dm.galenoNovedades.bandeja(),
    ])
  ...
  const sincronizar = async () => {
    ...
    setAviso(null)
    const resultado = await window.dm.galenoNovedades.sincronizar()
    if (resultado.ok) {
  ...
  const aplicar = async (fila: FilaDeBandejaDeGaleno, clienteId: number | null, crearElCliente: boolean) => {
    ...
    setAviso(null)
    const resultado = await window.dm.galenoNovedades.aplicar(fila.novedad.id, clienteId, crearElCliente)
    if (resultado.ok) {
  ...
  const descartar = async (fila: FilaDeBandejaDeGaleno) => {
    setTrabajando(fila.novedad.id)
    const resultado = await window.dm.galenoNovedades.descartar(fila.novedad.id, 'Descartada desde la bandeja.')
    if (resultado.ok) {
...
function Cabecera({
  ...
  estado: EstadoDeGalenoNovedades | null
