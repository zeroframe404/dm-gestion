...
//
// 14.0: el long-poll del vigía se fue —lo reemplazó el canal en vivo— pero la lógica de qué bajar es
// la misma y vive en `vivo/grilla.ts`. Acá se la prueba SIN el socket en el medio: `unaVuelta` le pasa
// a `aplicarFotoDeLaGrilla` la misma foto que el simulador difunde por el canal
// (`simulador.fotoDeLaGrilla()`). Es a propósito que el socket no esté: lo que se prueba acá es la
// decisión de qué bajar y cuándo darlo por visto, y meterle un WebSocket al medio sólo agregaría
// esperas y ruido a un fallo que en realidad es de esta decisión. El camino completo, con socket y con
// dos computadoras de verdad, lo cubre `vivo.prueba.ts`.
//
...
/**
 * Una vuelta del aviso en vivo: aplicar la foto que el servidor difunde cuando algo cambia.
 *
 * Es exactamente lo que hace el canal al recibir un frame `grilla` (y al recibir la `bienvenida`, que
 * trae la misma foto con más campos alrededor): `aplicarFotoDeLaGrilla` compara lo que la foto dice
 * contra lo que esta computadora tiene por visto y baja nada más lo distinto. Devuelve qué pestañas se
 * bajaron.
 */
async function unaVuelta(simulador: VpsSimulado, motor: MotorDeSincronizacion): Promise<string[]> {
  return aplicarFotoDeLaGrilla(simulador.fotoDeLaGrilla(), motor)
}
...
async function unaComputadora(simulador: VpsSimulado): Promise<Computadora> {
  // `credencialesDelPuente` fuera de Electron lee estas dos: con esto lo que la foto manda a traer —las
  // métricas que el servidor ya calculó— sale contra el simulador y no contra el VPS de verdad.
 ...
 * relojes del motor siguen corriendo entre pruebas: sin esto, una vuelta puede encontrarse con la base
 * de otra prueba, o con ninguna.
 */
...
async function ponerseAlDia(simulador: VpsSimulado, pc: Computadora): Promise<void> {
  const bajadas = await unaVuelta(simulador, pc.motor)
...
test('aviso en vivo: baja sólo la pestaña que cambió, y recién ahí la da por vista', async () => {
  ...
  try {
    ...
    en(pc.db)
    await ponerseAlDia(simulador, pc)
    ...
    assert.deepEqual(await unaVuelta(simulador, pc.motor), ['AGOSTO 2026'], 'una sola pestaña, no las tres')
    ...
    // servidor», y la que la barra usa para «sincronizado hace…». Con el canal andando casi todo llega
    // por acá, así que si no la corriera las dos dirían una hora vieja con datos nuevos.
    ...
    simulador.pestanasLeidas = []
    assert.deepEqual(await unaVuelta(simulador, pc.motor), [])
    ...
    //    la escritura, esta computadora se despertaría por su propio cambio y se bajaría la planilla
    //    entera —dos mil y pico de renglones— para no encontrar nada.
    await pc.fuente.escribirCeldas([{ titulo: 'AGOSTO 2026', fila: 3, columna: 4, valor: '13000', id: 'id-gomez' }], {
      'AGOSTO 2026': 5,
    })
    simulador.pestanasLeidas = []
    assert.deepEqual(await unaVuelta(simulador, pc.motor), [], 'reconoce su propia escritura')
    ...
    assert.deepEqual(
      await unaVuelta(simulador, pc.motor),
      ['AGOSTO 2026'],
    ...
    const conLaNueva = await unaVuelta(simulador, pc.motor)
    ...
    const tras = await unaVuelta(simulador, pc.motor)
...
test('aviso en vivo: lo que no se pudo bajar NO se da por visto', async () => {
  ...
  try {
    ...
    pc.motor.apagar()
    assert.deepEqual(await unaVuelta(simulador, pc.motor), [], 'no bajó nada')
    ...
    pc.motor.encender()
    assert.deepEqual(await unaVuelta(simulador, pc.motor), ['AGOSTO 2026'], 'lo reintenta solo')
